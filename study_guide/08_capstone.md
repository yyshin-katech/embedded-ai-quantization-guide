...
> 🔬 **실측 반영 (2026-08-18 — 커스텀 op export의 진짜 벽 = 트레이서 사망):** BEVDet를 TRT 플러그인 경로로 export할 때 실제로 죽는 지점은 `grid_sample`이 아니라 `TRTBEVPoolv2.forward` 내부의 nested `QuickCumsumCuda`였다 — `symbolic`이 없는 CUDA `autograd.Function`이라 torch 1.13 JIT 트레이서가 그래프에 기록하려다 `RuntimeError: _Map_base::at` + segfault. **해법은 표준 mmdeploy 기법**: export 동안 `forward`를 shape-correct `new_zeros`로 갈아끼우면(실 CUDA 연산 미실행) 트레이서가 죽지 않고, `symbolic`은 그대로 `mmdeploy::bev_pool_v2` 노드를 emit하며, 실연산은 **추론 시 TRT 플러그인**이 수행한다(엔진 정확도 무영향). 커스텀 op은 forward가 아니라 symbolic이 그래프를 만든다는 원칙. 상세 §4-4 후속 콜아웃 + [INT8 보고서](../logs/stage8_capstone_int8_report.html) §3.
...
> 🔬 **실측 반영 (2026-08-18 — 1차 스코프: FP32 walking skeleton):** 1차 실기 검증은 사용자 합의로 **FP32 walking skeleton까지**(§4-2)를 목표로 했고, 아래 INT8 경로는 그 시점엔 벽이라 **다음 과제로 남겼다**. **경로 A1**(`convert_bevdet_to_TRT.py --int8`)·**경로 B**(DerryHub)는 `bev_pool_v2`/deformable attn의 **커스텀 TRT 플러그인(`TRTBEVPoolv2`)을 TensorRT 8.5 + CUDA 11.x 툴체인으로 빌드**해야 하는데(정본 호스트는 TRT 10.16), 이는 §4.6 BEVFormer가 부딪힌 "포크 커스텀 op 플러그인 툴체인" 벽과 **동일**하다. **경로 A2**(ModelOpt QDQ)는 `bev_pool_v2`가 표준 ONNX op이 아니라 export 자체가 §4-3 벽(커스텀 심볼릭 필요).

> 🔬 **실측 반영 (2026-08-18 — 후속 세션: 경로 A1 INT8 관통 ✅):** 위에서 "다음 과제"로 남긴 **경로 A1**(바로 아래 `convert_bevdet_to_TRT.py --int8`)을 후속 세션에서 **user-space에 TRT-8.5-plugin 툴체인을 실제 조립해 관통**시켰다 — 커스텀 `TRTBEVPoolV2` 플러그인을 풀 mmdeploy CMake 없이 2개 TU만 직접 컴파일(**W3**, `libmmdeploy_tensorrt_ops.so` 58,896 B), torch 1.13 트레이서가 nested `QuickCumsumCuda`(symbolic 없는 CUDA autograd.Function)에서 죽는 걸 **export 전용 `new_zeros` shim**으로 우회(**W5** — symbolic만 `mmdeploy::bev_pool_v2` 노드 emit, 실연산은 추론 시 플러그인이 수행), deprecated `build_engine` segfault를 `build_serialized_network`로 교체(**W6**, 플러그인 탓 아닌 API 탓). INT8 캘리브는 ENTROPY_2·mini 81장. → **FP32→FP16→INT8 TRT 엔진 3종**을 빌드해 지연 사다리를 실측(batch1 event-timed): **지연** FP32 `14.68ms` → FP16 `4.91ms`(**×2.99**) → INT8 `2.63ms`(**×5.58**, FP16 대비 ×1.87); **엔진 크기** `245.0`→`90.1`(×2.72↓)→`46.7 MB`(×5.24↓). FP32 대비 출력 corr는 FP16 ≥0.9994(무시할 수준)·INT8 0.985~1.000(height head 0.985 최저, dim head rel_max 0.218 최대 — 양자화 오차 전파). 교차확증: TRT FP32 엔진 14.68ms는 PyTorch eager(§4-2 실측) 34.06ms보다 **2.3× 빠름**(커널 퓨전·파이썬 오버헤드 제거). **⚠️ 캐비앗 불변**: init 가중치(Baidu-locked)라 **절대 mAP≈0은 무의미** — 유효 산출물은 **가중치-무관 지연 사다리·엔진 크기·FP32대비 출력편차**뿐이고, 절대 정확도는 정식 가중치 확보 시 동일 하네스로 재실행 가능. 전체 서술·SVG·판정은 [`logs/stage8_capstone_int8_report.html`](../logs/stage8_capstone_int8_report.html), 벽·레시피·설계규칙은 [`capstone_constraints.md`](../experiments/stage8_capstone/capstone_constraints.md) 벽 5. (경로 A2/B는 미시도 — 이번 관통은 경로 A1 한정. 이 머신의 다른 INT8 축은 [3단계](05_tensorrt.md)·[5단계](07_infrastructure.md) ResNet50·[4단계](06_multi_soc.md) Hexagon HTP에서 완결.)
...
> 🔬 **실측 반영 (2026-08-18 — 후속 세션: 판정 기준 ② INT8 축 실측):** 위 뼈대에 **INT8 층을 실제로 붙였다**(§4-4 경로 A1 관통). 판정 기준 ②의 (FP32, FP16, INT8) × TensorRT dGPU 표에서 **지연·메모리 열이 실측으로 채워짐** — FP32 14.68ms/245MB → FP16 4.91ms(×2.99)/90MB → INT8 2.63ms(×5.58)/47MB. 단 init 가중치라 mAP/NDS 열은 여전히 폴백(정식 가중치 대기)이고, mixed·멀티타깃(판정 ③)은 미시도로 남는다. 커스텀 `TRTBEVPoolV2` 플러그인을 user-space에 조립한 6벽(W1~W6) 서사는 [`logs/stage8_capstone_int8_report.html`](../logs/stage8_capstone_int8_report.html).
...
- [TexasInstruments/edgeai-tidl-tools](https://github.com/TexasInstruments/edgeai-tidl-tools) · [operators.md](https://github.com/TexasInstruments/edgeai-tidl-tools/blob/master/docs/operators.md) · [quantization.md](https://github.com/TexasInstruments/edgeai-tidl-tools/blob/master/docs/quantization.md) — TIDL 지원 op·`TIDLCompilationProvider`/`TIDLExecutionProvider`·미지원 op의 Cortex-A offload. ⚠️ 구 링크 `docs/supported_ops_rts_versions.md`는 404(태그 `11_02_16_00` 재편에서 삭제) — `operators.md`로 대체됐다.
