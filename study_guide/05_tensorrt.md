...
> **🔬 실측 (온디바이스·새 모델 축): 같은 Orin에 트랜스포머(DETR)를 올리면 DLA 규칙이 어떻게 갈리는가** — 위 실측은 전부 **깨끗한 CNN(ResNet50)**. 모델을 `facebook/detr-resnet-50`(CNN 백본 + 트랜스포머 encoder/decoder)로 **바꿔** 같은 보드·같은 `trtexec`·같은 플래그로 재측정했다(2단계 §4.5 자산 재사용). 세 발견이 위 CNN 결론을 **모델 축에서 정밀화**한다.
> - **① "작은 iGPU는 INT8 무이득"은 모델 의존이었다(위 §2.3 line 187 정밀화).** ResNet50@224는 launch/메모리 바운드라 iGPU INT8/FP16 = **0.984(이득 0)**였다. DETR@800×1066은 어텐션 대형 matmul로 **compute 바운드** → iGPU FP32 **25.85** → FP16 **13.28(×1.95)** → INT8(implicit) **9.43 ms(×2.74, FP16 대비 ×1.41 빠름)**. **INT8 이득의 유무는 iGPU가 아니라 모델의 연산 강도가 정한다.** (DETR은 같은 보드 ResNet50보다 **지연**이 9~13× 크다 — 정밀도별 ×13.3/×12.9/×9.3; 엔진 파일 크기 비가 아님.)
> - **② accuracy-valid INT8이 파싱 자체를 못 한다(3단계 case C를 실 트랜스포머가 재현).** ORT `quantize_static(QInt8)` 산출 `detr_int8.onnx`(zp≠0 1085/1485 Q·DQ + 149 Conv/Gemm 전부 INT32 bias DQ)를 `trtexec` 직접 파서에 넣으면 **노드 0에서 0.05초 만에** `Assertion failed: shiftIsAllZeros(zeroPoint)` — 3단계 §2.2.1의 **case C(zp≠0)** 가 통념이 아니라 실제로 재현. 같은 보드 ResNet50 INT8 QDQ가 빌드된 건 **대칭(zp=0) export**였기 때문 → 같은 도구·다른 export 정책이 결과를 가른다. 정확도 유효 INT8은 대칭 재양자화/ORT-EP(§2.2.1) 필요. 위 iGPU INT8 9.43 ms는 **implicit(자동 레인지)라 지연 전용**.
> - **③ DLA에 트랜스포머는 부적합 — 이론 line 180/실습5 line 892의 "파편화" 실사례(헤드라인).** ResNet50에 깨끗한 **2/2 오프로드**(GPU 폴백 2층)를 준 그 recipe(`--useDLACore=0 --allowGPUFallback`)가 DETR에선 **DLA 326층 / GPU 폴백 404층 / 16조각**. 폴백 404층 = 트랜스포머 비-DLA 연산(**250 SHUFFLE**[어텐션 reshape]·34 MATRIX_MULTIPLY[Q·Kᵀ]·**30 NORMALIZATION**[LayerNorm, NVDLA v2 미지원]·12 SELECT[마스크 Where]) → DLA 적격 연산을 16개 섬으로 쪼개 경계마다 DLA↔GPU 복사. 결과 **DLA FP16 398.6 ms = iGPU FP16(13.28)보다 30× 느림**; DLA INT8(78.48 ms)로 켜도(위 "DLA=INT8 전용기" 규칙은 남은 섬에서 유지·×5.08 빠름) 여전히 **iGPU INT8보다 8.3× 느림**. **설계 규칙(일반화): DLA는 DLA 서브그래프의 GPU 폴백이 ~0일 때만 이득 — DLA는 CNN 가속기다. 트랜스포머 detection 헤드는 iGPU에, DLA엔 순수 CNN(백본·분류기)만.** (이론 line 180 "20~30% 폴백이면 이점 소멸"의 극단 실사례 — DETR은 폴백 55%.)
>
> 📄 전체 실측·SVG·판정: [`../logs/stage3_jetson_orin_detr_report.html`](../logs/stage3_jetson_orin_detr_report.html) · 데이터·스크립트·README: [`../experiments/stage3_tensorrt/jetson_ondevice/detr/`](../experiments/stage3_tensorrt/jetson_ondevice/detr/)
> **캐비앗:** 위 캐비앗 전부 승계(지연 event-timed·batch1·MAXN·상대만, iGPU/DLA INT8은 implicit라 지연·크기만 유효·정확도 미주장). DETR 정확도는 온보드 미측정(COCO 부재) → **2단계 RTX 인용**: FP32 mAP 0.4207 → INT8(ORT QDQ) 0.2402(−42.9%), mAP_s −77%([2단계 §4.5](04_transformer_quantization.md)). DLA 파편화 수치(404/16)는 DETR op 믹스 고유이나 **규칙**(폴백 수가 DLA 이득을 좌우)은 일반화.
>
> **🔬 실측 (온디바이스·정확도 축): "정확도 미주장" 캐비앗을 닫는다 — Orin 실리콘에서 INT8은 살아남는가** — 위 §2.3 실측은 전부 **지연·전력**이었고 매번 "정확도 미주장"을 달았다. 이번엔 저장된 `.plan`을 **4단계 CPU-프록시가 쓴 바로 그 ResNet50 1000장 번들**에 돌려 이미지별 예측을 1:1 대조했다(러너 = `tensorrt` Python API + `cuda-python` 12.9.7 `cuda.bindings.runtime`, batch1). 결정적 장치: `rn50_gpu_int8.plan`은 CPU 프록시가 ORT에 먹인 **바로 그 `resnet50_int8_qdq.onnx`**에서 빌드돼 **QDQ scale이 동일** → 유일한 변수는 정수커널 데이터패스다.
> - **① accuracy-valid INT8은 온디바이스에서 유지된다(게다가 CPU 커널을 이김).** iGPU INT8(explicit QDQ, 실 scale) top-1 = **0.7620 = iGPU FP32**(1000장 무손실), **CPU MLAS INT8(0.7500)보다 높다**. 이로써 위 line 193 캐비앗의 "정확도는 iGPU explicit INT8에서 확립"이 **온디바이스로 입증**된다. (무음 FP32 폴백 아님 — INT8은 자기 FP32와 예측이 57장 다르고, 지연도 1.01≠1.94 ms.)
> - **② INT8 예측 경로 의존이 CPU↔가속기 경계를 넘는다(4단계 확장).** TRT INT8(GPU 정수커널)과 MLAS INT8(CPU SDOT)은 **같은 QDQ scale·같은 실리콘**인데 **961/1000(39장 불일치)**. 이 96.1%는 4단계 Jetson↔x86 CPU 교차(958/1000 = 95.8%)와 **같은 대역**이고 Jetson↔Pi5 100%(같은 MLAS SDOT)가 아니다 → **정수커널이 다르면 ~96%, 경계가 CPU↔CPU든 CPU↔가속기든 무관**. 39장 중 TRT 정답 15·MLAS 정답 3·둘다오답 21(TRT가 더 충실). **FP32는 경로 무관 — iGPU FP32 vs CPU FP32 = 1000/1000**(4단계 FP32 100%가 가속기까지 확장). [4단계 §2-2](06_multi_soc.md#2-2--실측-부분프록시-arm-cortex-a-폴백-바닥값--pi-5a76--imx8m-nanoa53--jetson-orina78ae)의 크로스플랫폼 표에 이 CPU↔가속기 행이 짝을 이룬다.
> - **③ implicit INT8은 0.017로 붕괴(캐비앗 정량화).** `rn50_dla_int8.plan`(=`trtexec --int8` 캘리브 없음, 자동 레인지) top-1 = **0.0170**(near-random). **같은 하네스**가 DLA FP16 = 0.7610을 주므로 하네스 버그가 아니다 — 위 실측들이 implicit/DLA INT8을 "정확도 미주장"으로 라벨한 이유를 **수치로 확증**한다. accuracy-valid INT8은 explicit-QDQ iGPU 엔진(①)뿐.
>
> 📄 전체 실측·SVG·판정: [`../logs/stage3_jetson_orin_accuracy_report.html`](../logs/stage3_jetson_orin_accuracy_report.html) · 데이터·스크립트·README: [`../experiments/stage3_tensorrt/jetson_ondevice/accuracy/`](../experiments/stage3_tensorrt/jetson_ondevice/accuracy/)
> **캐비앗:** 절대 top-1은 **1000장 서브셋**(1단계 함정 0: 부풀림)이라 3단계 5000장 RTX 수치와 비교 불가 — **같은 번들 상대 관계**(일치율·INT8 vs FP32·TRT vs MLAS)만 결과. `dla_int8`(0.017)은 implicit이라 캐비앗 정량화용일 뿐 DLA 정확도가 아님. QDQ scale은 3단계 `resnet50_int8_qdq.onnx`에서 동결(재캘리브 없음).

> **🔬 실측 (온디바이스·정확도 축·DETR): DETR "정확도 미측정" 캐비앗을 닫는다 — 대칭 재양자화로 case C를 우회하면 accuracy-valid INT8은 살아남는가** — 위 **DETR 지연 콜아웃**은 "정확도 미주장"을 달고 2단계 RTX만 인용했다(그 발견 ②처럼 ORT 비대칭 `detr_int8.onnx`가 **case C(zp≠0)로 파싱조차 안 됐기** 때문). 이번엔 3단계 ResNet50이 빌드됐던 그 **대칭 레시피**로 재양자화해 accuracy-valid INT8 엔진을 **처음으로 존재하게** 만들고, COCO val 1000장(고정 800×1066)에서 온보드 pycocotools mAP를 잰다.
> - **① 두 벽을 넘어 explicit INT8이 빌드된다(툴체인 언락).** (1) **case C+B 해소**: `ActivationSymmetric+WeightSymmetric`(zp=0 → C 해소)·`QuantizeBias=False`(INT32 bias DQ 제거 → B 해소)·backbone `conv1` 제외(case D) → `detr_int8_sym.onnx`(43.4 MB, **zp_nonzero=0·int32_bias_DQ=0**). (2) **트랜스포머 고유 2차 벽**: 파싱은 통과해도 빌더가 self-attention의 quantized-constant를 거부(`qdqGraphOptimizer::matchQuantizedConstantPluginOrDQ … Quantized constant is only allowed before DQ or PLUGIN`) → `op_types_to_quantize=[Conv,Gemm]`(가중치 op만; attention matmul·LayerNorm·Softmax는 FP16 유지)로 넘는다 = **2단계 §4.5**("op 선택 mixed는 mAP를 거의 못 움직인다")와 정확히 일치. 결과 9ef2a58에서 **build-failed였던** explicit INT8이 빌드됨(**44.69 MiB·11.002 ms**, FP32 대비 ×2.35·FP16 대비 ×1.21).
> - **② 그러나 case-C 픽스는 툴체인 언락이지 정확도 구제가 아니다(헤드라인).** accuracy-valid INT8은 2단계 폭락을 **재현**: FP32 mAP **0.4237** → INT8 **0.2383(−43.8%)**, 작은 객체 mAP_s 0.2179→**0.0336(−84.6%)**. 2단계(ORT-QDQ·dynamic·CUDA·5000장) 0.4207→0.2402(**−42.9%**)·mAP_s −77% — **다른 두 경로(TRT/고정shape/1000 vs ORT/dynamic/5000)가 같은 폭락에 착지** → 교차확증. 레버는 대칭/case-C 픽스가 아니라 **activation 양자화 입도**(SmoothQuant, [2단계 §4.4](04_transformer_quantization.md); per-tensor 격차의 59.9% 회복).
> - **③ implicit `--int8`(0.4073)은 더 나은 INT8이 아니다 — 정확도가 nominal.** `trtexec --int8`은 캘리브 캐시 없이 빌드돼 `[W] Calibrator is not being used…`를 찍는다 → **데이터 기반 activation 레인지가 없다**. 그래도 INT8 커널은 돈다(implicit 9.43 ms는 FP16 13.28 ms보다 **빠르고** 엔진도 58.76 < 81.43 MiB로 **작다** → 순수 FP16 폴백이 아님) — 단 **무통제 레인지** 위에서라 정확도를 걸 수 없다(위 DETR 지연 콜아웃의 iGPU INT8 9.43 ms가 이 implicit이라 **지연 전용**이었던 이유). 여기선 우연히 FP16 근처(0.4073)지만, 짝인 위 정확도 축 콜아웃 ③의 ResNet50-DLA implicit는 같은 무통제 경로에서 0.017로 붕괴 — 무통제 경로는 어느 방향으로든 정확도를 보장하지 못한다.
>
> 📄 전체 실측·SVG·판정: [`../logs/stage3_jetson_orin_detr_accuracy_report.html`](../logs/stage3_jetson_orin_detr_accuracy_report.html) · 데이터·스크립트·README: [`../experiments/stage3_tensorrt/jetson_ondevice/detr_accuracy/`](../experiments/stage3_tensorrt/jetson_ondevice/detr_accuracy/)
> **캐비앗:** 절대 mAP는 **2단계와 비교 불가**(고정 800×1066 강제 리사이즈 vs dynamic shape·TRT vs ORT·1000 vs 5000장) — **상대 FP32→INT8 폭락만** 결과이고 그 상대값이 2단계를 교차확증한다. 1000장 서브셋(1단계 함정 0). `int8_implicit`(0.4073)은 무통제 자동 레인지라 정확도 미주장. 전역 logits corr(sym 0.982 ≈ implicit 0.980)은 no-object 차원 지배라 **약한 프록시** → pycocotools mAP가 권위 지표.
