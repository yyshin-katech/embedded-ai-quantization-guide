...
> 🔬 **온디바이스 후속(2026-08-21, Jetson AGX Orin · TRT 10.3.0 · ONNX→TRT).** 위 59.9%는 **torch fake-quant 경로(Design X)에서만** 나온 값이다 — 그 경로는 ONNX를 낳지 않아 온디바이스로 올린 적이 없다. 이번에 SmoothQuant를 **ONNX 그래프에 직접**(95개 Gemm, per-input-channel `s=a^α/w^(1−α)`, FP32-정확 게이트 1.7e-4 · **CPU EP**[Ampere TF32가 재스케일에 민감]) 심어, [DETR 정확도 축](../logs/stage3_jetson_orin_detr_accuracy_report.html)(커밋 `58ba518`)이 재현한 폭락(FP32 **0.4237** → 대칭 INT8 **0.2383**, gap **0.1854**) 위에 얹었다. **결과 반전**: 온디바이스에선 폭락의 **9.2%(α=1.0 → 0.2553)** · **8.7%(α=0.5)**만 회복 — 위 stage2 **59.9%보다 약 6.5배 작다**. **근본 원인 = op 커버리지**(SmoothQuant가 틀린 게 아니다): 온디바이스에서 빌드 가능한 유일한 INT8 엔진은 case-C 파서 + quantized-constant 빌더 벽 때문에 `op_types_to_quantize=[Conv,Gemm]`로 강제돼 attention MatMul · LayerNorm · Softmax가 **FP16으로 남는다** → INT8이 **Gemm 전용**이라 Gemm 입력 smoothing은 이 경로의 지배적 잔차 오차원이 아니다. 반면 stage2 torch 경로는 attention Q/K/V/out을 **포함한 모든 linear**가 per-tensor INT8이라 SmoothQuant가 크게 먹혔다. 단 SmoothQuant는 **방향이 옳다**: α=1.0(9.2%) > α=0.5(8.7%)로 stage2의 DETR-best α 순서를 보존하고, 폭락이 집중된 작은 객체 mAP_s가 0.0336 → 0.0449(gap의 6.1%)로 부분 회복하며, 대가는 activation Mul 95개뿐이라 **거의 지연 중립**(+0.34 ms · 엔진 +0.18 MiB, activation absmax는 최대 46.7×/중앙 7.49× 감쇠). **∴ 위 59.9%는 부정되는 게 아니라 "경로·op-커버리지 의존"으로 정밀화된다** — 레버가 싸다(공짜)는 것과 강력하다는 것은 별개이고, 강력함은 무엇이 INT8인가에 달렸다. 상세·SVG: [온디바이스 SmoothQuant 리포트](../logs/stage3_jetson_orin_detr_smoothquant_report.html) · 스크립트: [`experiments/stage3_tensorrt/jetson_ondevice/detr_smoothquant/`](../experiments/stage3_tensorrt/jetson_ondevice/detr_smoothquant/). ⚠️ 절대 mAP는 fixed 800×1066 · 1000장 · TRT라 위 stage2 5,000장 · torch 절대값과 **1:1 비교 불가** — 같은 번들 안의 **상대 회복률만** 결과.
