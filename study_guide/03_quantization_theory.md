...
> ```
>
> 즉 **후보 threshold가 "히스토그램 전체 범위" 하나뿐**이라 argmin이 자동으로 전체 범위 = **MinMax와 같은 결과**를 낸다. 위 의사코드의 `for i in range(128, 2048)`이 ORT 기본값에서는 `for i in range(128, 129)`인 셈이다. 실측으로도 activation scale·zero_point가 MinMax와 **32/32 텐서 전부 비트 단위로 동일**했고 1000장 예측 불일치가 **0장**이었다(그런데 시간은 9.0s → 25.1s로 **2.8배** 더 씀). 대응은 4.3 참조.
...
2.4.3에서 본 KL 탐색 알고리즘은 **ORT 기본값으로는 실행되지 않는다**. `EntropyCalibrater`의 기본 `num_bins=128, num_quantized_bins=128` 때문에 탐색 후보가 정확히 1개(=전체 범위=MinMax)로 줄어든다. 실측에서 Entropy 산출물은 MinMax와 **activation scale이 32/32 전부 동일**했고 1000장 예측 불일치가 **0장**이었다 — **결과는 같은데 시간만 2.8배**(9.0s → 25.1s) 쓴 셈이다. 진단 코드는 2.4.3의 `check_entropy_degenerate.py`.
...
> ```
>
> `create_calibrator()` 자체는 `num_bins`/`num_quantized_bins`를 **받는데**, `quantize_static()`이 안 넘겨준다. 에러도 경고도 없이 무시된다.
...
> 🔴 **함정 (고친다고 좋아지지 않는다 — 오히려 크게 나빠졌다)**: 위 몽키패치로 2.4.3의 알고리즘을 **제대로 돌린** 결과는 top-1 **67.70%(−10.80%p, McNemar p<0.0001로 확실한 열화)**, 시간은 51.0s로 5.7배(top-1 절대값은 3.1절의 큐레이션 셋 기준이라 부풀려져 있다 — **%p 변화만** 읽을 것). 즉 "ORT의 Entropy는 망가져 있으니 고쳐 쓰라"가 아니라, **"이 모델에서는 KL이 고르는 임계가 MinMax보다 해롭다"** 가 실측 결론이다. 이유는 5.1과 같다 — ResNet18의 post-ReLU 꼬리는 잡음이 아니라 신호라서, KL이 "정보 보존"을 명분으로 꼬리를 잘라도(표현 상한이 MinMax 대비 **0.313배**) 정확도는 따라오지 않는다. **알고리즘의 우아함과 이 모델에서의 유용함은 별개다.**
>
> 실전 결론: ORT에서 Entropy를 지정하는 것은 **기본값이면 MinMax와 동일(시간만 낭비)**, **제대로 열면 이 모델에선 손해**다. 진짜 KL 캘리브레이션이 필요하면 TensorRT의 `IInt8EntropyCalibrator2`(3단계)나 NVIDIA Model Optimizer를 쓰고, ORT에서는 **MinMax를 기본으로 두고 Percentile 99.999부터 스윕**하는 편이 낫다.

#### 4.3.2 타깃별 dtype 분기표 — 이 설정은 x86 전용이다

위 4.3 코드의 `activation_type=QUInt8` + `ActivationSymmetric=False`(비대칭)는 **x86 CPU 권장 설정**이고, **TensorRT에서는 그래프가 아예 파싱되지 않는다**. 실측에서 이 QDQ를 TensorRT EP에 물렸더니 노드를 하나도 가져가지 못하고 전부 폴백해 **FP32(0.95~1.03 ms)보다 3배 느린 3.05 ms**가 나왔다 — 에러 없이, 로그를 켜지 않으면 보이지도 않는 **무음 열화**다.
...
> 이 표는 아래 "실측 효과" 표와 **별도 실행**(4개 모델 연속, 워밍업 20 + 측정 60회 p50)이라 절대값이 소수점에서 다르다(C 0.52 ↔ 아래 0.55). 여기서 읽을 것은 값이 아니라 **✅/🔴의 배치**다. 재현 스크립트는 [실습 로그 8장](../logs/stage1_quantization_log.html#s8).

INT32 bias DQ를 **0개로 없애도 실패**하고(B), **21개가 남아 있어도 성공**한다(C). 즉 **하드 블로커는 ② zero-point 하나**이고, **① 은 융합이 깨진 뒤 홀로 남은 bias DQ가 타입 검사에 걸리는 2차 증상**이다. 실무적으로: `activation_type`과 `ActivationSymmetric`을 **같이** 뒤집는 것이 유일한 해법이고, `"QuantizeBias": False`는 그래프를 정리하는 **선택 옵션**(0.52 → 0.51 ms, 오차 범위)이지 해법이 아니다.
...
| 설정 | TRT p50 | vs FP32 | top-1 | vs FP32 | McNemar p |
|------|---------|---------|-------|---------|-----------|
| FP32 | 1.03 ms | 1.00× | 78.50% | — | — |
| INT8 `QUInt8` 비대칭 (위 4.3 기본 설정) | **3.05 ms** | **0.34× (느려짐)** | 78.90% | +0.40%p | 0.4795 |
| INT8 `QInt8` 대칭 | **0.55 ms** | **1.87×** | 78.10% | −0.40%p | 0.5224 n.s. |
| INT8 `QInt8` 대칭 + `QuantizeBias=False` | **0.51 ms** | **2.02×** | 78.10% | −0.40%p | 0.5224 n.s. |

> 위 top-1은 **클래스당 1장 큐레이션 셋(1000장)** 기준이라 FP32가 78.50%로 ImageNet val 공식 69.76%보다 높다 — **절대값이 아니라 상대 비교로만** 읽을 것(3.1절).

**설정 하나로 5.5배가 갈린다**(3.05 → 0.55 ms). 반면 대칭으로 바꾼 정확도 대가는 **−0.4%p로 통계적으로 유의하지 않다**(McNemar p=0.5224). 2.2에서 "activation은 비대칭이 이득"이라고 배운 그 이득은, **TensorRT 타깃에서는 애초에 선택지가 아니며 붙잡고 있으면 손해만 본다.**
...
def evaluate(onnx_path, items, n=1000):
    ...
    hits = np.zeros(min(n, len(items)), dtype=bool)
    for i, (path, label) in enumerate(items[:n]):
...
if __name__ == "__main__":
    ...
    for name, path in [
        ...
        hits = evaluate(path, items, n=1000)
        results[name] = hits
...
> 💡 **팁**: `evaluate()`가 정답 개수 대신 **이미지별 정오답 bool 배열**을 돌려주게 한 것이 핵심이다. 이 배열만 `np.savez`로 남겨 두면 재추론 없이 **부분집합 재채점**(예: 캘리브에 쓴 이미지를 뺀 holdout 점수)과 **모든 모델 쌍의 paired 검정**을 나중에 다시 할 수 있다. 실측에서도 캘리브 200장을 제외한 holdout 800장 점수가 전체 1000장과 **0.3%p 이내로 일치**하는지 이 방식으로 확인했다(중복이 결론에 영향 없음을 검증).
...
>
> **왜 McNemar가 따로 필요한가 (실측):** Wilson CI는 두 모델을 **독립 표본**처럼 다루지만, 실제로는 **같은 1000장을 둘 다 본다**. 같은 이미지를 둘 다 맞히거나 둘 다 틀리는 부분은 비교에 정보를 주지 않으므로, **엇갈린 이미지만** 세는 McNemar가 훨씬 민감하다. 실측 예: per-channel vs per-tensor는 Wilson CI가 `[76.26, 81.32]` vs `[75.02, 80.17]`로 **거의 완전히 겹쳐** CI만 보면 아무 말도 못 하지만, McNemar로 보면 엇갈린 이미지가 **34장뿐**(per-ch만 맞음 23 / per-tensor만 맞음 11)이라 차이의 방향과 크기를 특정할 수 있다(p=0.0592). 반대로 Percentile 99.9는 p<0.0001로 **명확히** 갈린다. **CI는 "말할 수 없음"만 알려주고, McNemar가 "어느 쪽이 얼마나"를 알려준다.**

> 🔴 **함정 (라벨 매핑)**: 위 `load_labels`는 "폴더명 정렬 순서 = torchvision 클래스 인덱스"라고 가정한다. 이 가정은 **폴더가 1000개 다 있을 때만** 성립한다. torchvision ResNet18의 인덱스가 ImageNet **synset ID(n0…)의 사전순 정렬과 일치**한다는 것은 이 실습에서 1000/1000으로 실측 확인했지만(3.1절), **클래스 서브셋(Imagenette 등)에서는 인덱스가 0부터 다시 매겨져 top-1이 0%로 무너진다** — 서브셋 대응 코드는 3.1절 참조. 어느 쪽이든 **먼저 FP32 점수부터 확인하는 sanity check**가 최선의 방어다(ImageNet val이면 ~69.8%, 3.1의 큐레이션 셋이면 ~78.5%).
...
> ⚠️ **이 표의 수치를 읽는 법 (중요)**: 아래는 이 스터디에서 **실제로 측정한 값**이지만, ImageNet val 공식 점수가 **아니다**. 평가셋은 `EliSchwartz/imagenet-sample-images`(**클래스당 1장 큐레이션 셋, 1000장**, 3.1절)이고 그중 5장 간격 200장을 캘리브에 썼다. 큐레이션된 대표 이미지라 **FP32가 78.50%로 ImageNet val 공식 69.76%보다 8.7%p 높게** 나온다. 따라서 **절대값은 인용하지 말고 "FP32 대비 상대 변화"만** 읽어야 한다. (캘리브 200장을 제외한 holdout 800장 점수도 전 모델에서 전체 1000장과 0.3%p 이내로 일치해, 캘리브/평가 중복이 아래 결론을 바꾸지 않음을 확인했다.) 95% CI는 Wilson, McNemar는 FP32와의 paired 비교(4.4). 전체 출력은 [실행 로그](../logs/stage1_quantization_log.html).

| 모델 | 캘리브레이션 | per-ch | top-1 | 95% CI | FP32 대비 | McNemar p |
|------|-------------|--------|-------|--------|-----------|-----------|
| **FP32 (기준)** | — | — | **78.50%** | [75.85, 80.93] | — | — |
| INT8 | **MinMax** | ✔ | **78.90%** | [76.26, 81.32] | **+0.40%p** | 0.4795 n.s. |
| INT8 | MinMax | ✘ (per-tensor) | 77.70% | [75.02, 80.17] | −0.80%p | 0.2299 n.s. |
| INT8 | Entropy (ORT 기본) | ✔ | 78.90% | [76.26, 81.32] | +0.40%p | 0.4795 n.s. |
| INT8 | Entropy (탐색 정상화 2048/128) | ✔ | **67.70%** | [64.74, 70.53] | **−10.80%p** | **<0.0001 유의** |
| INT8 | Percentile 99.9 | ✔ | **72.30%** | [69.44, 74.98] | **−6.20%p** | **<0.0001 유의** |
| INT8 | Percentile 99.99 | ✔ | 78.10% | [75.43, 80.55] | −0.40%p | 0.5403 n.s. |
| INT8 | Percentile 99.999 (ORT 기본) | ✔ | 78.60% | [75.95, 81.03] | +0.10%p | 1.0000 n.s. |
| INT8 | MinMax, **QInt8 대칭**(TRT용) | ✔ | 78.10% | [75.43, 80.55] | −0.40%p | 0.5224 n.s. |

**해석**:

- ResNet18 정도의 잘 정규화된 CNN은 INT8 PTQ만으로도 **손실이 사실상 없다**. MinMax는 오히려 +0.40%p 높게 나왔지만 McNemar p=0.4795로 **FP32와 구분되지 않는다** — "INT8이 FP32보다 좋아졌다"가 아니라 **"차이를 검출할 수 없다"** 가 정확한 서술이다.
...
- **클리핑 양과 정확도는 거의 완전한 단조 관계다.** activation 표현범위 상한이 MinMax 대비 몇 배인지(중앙값)와 top-1을 나란히 놓으면:
  ...
  | 캘리브레이션 | 상한 비율(MinMax=1.0) | 0.5배 미만으로 잘린 텐서 | top-1 |
  |---|---|---|---|
  | MinMax | 1.000 | 0 / 32 | 78.90% |
  | Entropy (ORT 기본) | 1.000 | 0 / 32 | 78.90% |
  | Percentile 99.999 | 0.667 | 3 / 32 | 78.60% |
  | Percentile 99.99 | 0.466 | 20 / 32 | 78.10% |
  | Percentile 99.9 | 0.310 | 29 / 32 | **72.30%** |
  | Entropy (2048 bin) | 0.313 | 20 / 32 | **67.70%** |

  **자를수록 나빠진다**, 예외 없이. 그리고 0.5배 근처에서 무너지기 시작한다. 이 표가 이 실습에서 가장 재사용 가치가 큰 결과물이다 — **새 모델에서 캘리브레이터를 고를 때 top-1을 재기 전에 "상한이 MinMax 대비 몇 배로 줄었나"부터 보면 된다.**
...
- **per-channel 대조**: MinMax를 per-tensor로 바꾸면 −1.20%p 나빠진다(78.90 → 77.70). 방향은 **2.2.2의 이론과 일치**하지만, 🔴 **크기는 유의 문턱을 넘지 못한다** — McNemar p=0.0592(엇갈린 이미지 34장: per-ch만 맞음 23 / per-tensor만 맞음 11), Wilson CI도 크게 겹친다. per-channel 이득을 확실히 보고 싶다면 **weight SQNR 쪽이 훨씬 선명하다**(평균 +7.38 dB, 최대 +14.06 dB — 4.5.2). **"top-1로는 흐릿하지만 SQNR로는 뚜렷하다"** 는 것 자체가 배울 점이다: 지표를 하나만 보면 실재하는 효과를 놓친다.

- ⚠️ **Entropy 두 행의 간극(78.90 vs 67.70)이 오타가 아니다.** 같은 `CalibrationMethod.Entropy`인데, ORT 기본값에서는 탐색이 죽어 **MinMax와 완전히 동일한 결과**(scale 32/32 비트 일치)가 나오고, 히스토그램 파라미터를 주입해 **알고리즘을 제대로 돌리면 −10.80%p로 무너진다**. 원인과 대응은 4.3.1.
