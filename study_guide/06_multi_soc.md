...
- **온디바이스 정확도 실측 — 위 지연·전력의 "정확도 미주장"을 닫고, §2-2의 경로 의존을 가속기로 확장.** 저장된 `.plan`을 §2-2 CPU-프록시가 쓴 **같은 ResNet50 1000장 번들**에 돌렸다. ① **iGPU INT8(explicit QDQ) top-1 = 0.7620 = iGPU FP32**(1000장 무손실), **CPU MLAS INT8(0.7500)보다 높다** → accuracy-valid INT8이 가속기에서 유지됨을 온디바이스로 입증(이 절 캐비앗의 "정확도는 명시적 QDQ iGPU INT8에서 확립"을 실측으로 채움). ② **TRT INT8 예측 vs 같은 실리콘 MLAS INT8 예측 = 961/1000**(같은 QDQ scale인데 39장 불일치) → §2-2의 크로스플랫폼 규칙("정수커널 다르면 ~96%, 같으면 100%")이 **CPU↔CPU를 넘어 CPU↔가속기 경계에서도 같은 대역으로 재현**(vs stage4 Jetson↔x86 958). **FP32는 iGPU↔CPU 1000/1000**(경로 무관 결정성이 가속기까지). ③ 단 **implicit DLA INT8은 0.017로 붕괴**(자동 레인지·같은 하네스가 DLA FP16 0.761은 정상) → "정확도 미주장" 라벨의 수치적 근거. 상세: [3단계 §2.3 정확도 콜아웃](05_tensorrt.md#23-dla-deep-learning-accelerator).

> 📄 전체 실측·SVG·판정: [`../logs/stage3_jetson_orin_ondevice_report.html`](../logs/stage3_jetson_orin_ondevice_report.html)(솔로 5-엔진) · [`../logs/stage3_jetson_orin_accuracy_report.html`](../logs/stage3_jetson_orin_accuracy_report.html)(온디바이스 정확도) · 데이터·스크립트·제약: [`../experiments/stage3_tensorrt/jetson_ondevice/`](../experiments/stage3_tensorrt/jetson_ondevice/) · TensorRT-측 DLA 이론+실측: [3단계 §2.3](05_tensorrt.md#23-dla-deep-learning-accelerator)
