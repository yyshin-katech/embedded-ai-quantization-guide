...
- 기준 커밋: `c22ba4c` (2026-08-06) · 마지막 갱신: 2026-08-10
- 이전 작업 머신: `Ubuntu 22.04.5` / `RTX 3060 12GB` / 드라이버 `595.84` (Neousys Nuvo-6108GC)
...
지금 **진행 중인 단 하나의 작업**은 1단계에서 파생된 **QAT 회복 실험**이고, 2팔 중 대조군을
한 번도 못 돌린 상태입니다. 막고 있는 것은 코드가 아니라 **이전 머신의 GPU 하드웨어 고장**
(Xid 79, 3회 재발)입니다. → §5, §6

**다른 PC로 옮기는 것 자체가 이 교착의 해결책일 수 있습니다.** 고장은 특정 카드/섀시에
국한된 증상이고, 실험 코드는 이식성 문제가 없습니다.
...
| QAT 실험 스크립트 | 17KB | ✅ | [`experiments/qat_recovery/`](experiments/qat_recovery/) |
...
> 0.06~0.14%p로 측정 노이즈 크기라 `1116.7%` 같은 무의미한 수가 나옵니다(스크립트에 경고 내장).

### 지금까지 실측된 값 (전부 부분 실행, 평가 10,000장)

| 설정 | FP32 | PTQ | QAT ep0 | QAT ep1 | 대조군 |
|---|---|---|---|---|---|
| BS=96 / calib 5,000 | 68.52% | 68.46% | 68.89% | 69.13% | ❌ 미실행 |
| BS=48 / calib 2,560 | 68.51% | 68.37% | 69.47% | ❌ GPU 사망 | ❌ 미실행 |

두 설정은 **서로 비교 불가**입니다 — step 수가 416→833으로 2배고, 캘리브 장수도 달랐습니다
(아래 함정 3). BS=48에서 FP32가 68.51%로 재현된 것은 분할·전처리·eval 모드가 같다는 확인입니다.
...
ls qat_recovery_result_bs48.json                                    # JSON 생성 확인
python3 qat_control_finetune.py 2>&1 | tee qat_control_bs48.log     # 팔 2 (~6분)
```

배치·에폭은 환경변수로 덮어씁니다: `QAT_BS_TRAIN=96 QAT_EPOCHS=2 python3 qat_recovery.py`.
**두 팔을 반드시 같은 설정으로** 돌리세요 — 팔 2는 JSON의 `(bs_train, epochs)`가 자기 설정과
다르면 실행을 거부합니다.
...
현재 구성(per-channel 대칭 INT8 weight + per-tensor 비대칭 UINT8 act)은 PTQ 손실이 애초에
무손실급이라 **"회복할 것이 없다"가 정직한 결론에 가깝습니다.** 회복률을 의미 있게 재려면
PTQ 손실이 큰 변형으로 바꾸세요 — Entropy 정규화(−9.45%p), Percentile 99.9(−6.83%p),
weight 4-bit 중 하나.
...
1. **GPU 교착 해소** — 새 PC라면 `nvidia-smi -L`만 확인하고 §5로. 같은 머신이면 §6의 전력
   상한을 먼저.
2. **QAT 2팔 재실행** — 같은 배치로 순차 실행, 2초 텔레메트리 병행. 팔당 ~6분.
3. **결과를 문서에 반영** — `03_quantization_theory.md` §2.5. 격차가 노이즈면 "QAT가 회복할
   손실이 애초에 없다"가 정직한 서술입니다. 억지로 이득을 만들지 마세요.
