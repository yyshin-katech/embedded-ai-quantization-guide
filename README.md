...
> 🔬 **2026-08-18~19, 4단계 ARM Cortex-A 폴백 바닥값을 실물 ARM 보드 2종으로 실측**했습니다(**부분·프록시** — 벤더 NPU가 아니라, 세 SoC가 공통으로 가진 **CPU 폴백 경로**를 측정). 3·5단계 자산 **ResNet50 INT8 QDQ**를 순수 `CPUExecutionProvider`로 관통시키니 **같은 INT8 그래프인데 CPU ISA가 양자화 이득의 부호를 뒤집습니다** — 그것도 **3-사분면**으로: Pi 5(Cortex-A76, `asimddp`/SDOT **있음**)는 INT8이 **144.95→79.08ms ×1.83 빨라지고**, x86 i9-10900K(Comet Lake, **VNNI 없음**)는 되레 **9.28→16.34ms 1.76× 느려지며**, **i.MX8M Nano(Cortex-A53·ARMv8.0·dotprod 없음)는 680.20→1123.02ms 1.65× 느려집니다**. **핵심은 부호를 가르는 게 "ARM이냐 x86이냐"가 아니라 dot-product 명령(SDOT/VPDPBUSD) 유무라는 것** — A76·A53은 같은 ARMv8 계열인데 부호가 반대이고(A53이 x86과 같은 "느려짐"), 통념 "ARM이면 INT8 유리"가 ARM 안에서 반박됩니다. 덤으로 크로스플랫폼 예측 동일성은 **FP32에서만**(세 쌍 모두 1000/1000=100%) 성립하고 **INT8은 imx↔pi5 965·imx↔x86 961·x86↔pi5 958(≈96%)** — INT8 정확도를 서로 다른 타겟에서 비트 단위로 기대하면 안 됩니다. 실물 저사양 보드(2GB no-swap·ORT 1.17.1)에선 **FP32 OOM**(→ lazy 전처리 `rpi_bench_lowmem.py`)과 **INT8 opset skew**(미사용 opset을 ORT 1.17.1이 거부 → strip) 두 벽도 채집했습니다. ⚠️ Pi·Nano는 폴백 프록시일 뿐 자동차 NPU가 아니며(Nano엔 NPU 자체가 없음 — Plus에만; 가속 수치 전이 불가), 절대값은 CPUEP·배치1·1,000장 서브셋 기준 **상대 관계만 유효**. NPU 실측은 보드 확보 시 4-A~C 과제(이 CPU 바닥값이 "가속기가 이겨야 할 최소선"). 실측 리포트 [`logs/stage4_arm_cpu_fallback_report.html`](logs/stage4_arm_cpu_fallback_report.html)(Pi 5·x86) · [`logs/stage4_imx8mn_a53_report.html`](logs/stage4_imx8mn_a53_report.html)(A53·3-사분면) · 데이터·스크립트 [`experiments/stage5_infrastructure/cpu_proxy/`](experiments/stage5_infrastructure/cpu_proxy/).
...
> 🔬 **2026-08-18, 캡스톤(BEVDet end-to-end)을 RTX 3080에서 FP32로 관통**시켰습니다(**1차 스코프** — 실제 FP32 baseline까지; **후속 세션에서 TRT-8.5-plugin INT8까지 관통** — 아래). 헤드라인은 **문서 §3의 빈칸을 메운 "제3의 길"**입니다: BEVDet의 `bev_pool_v2`(LSS pooling, python fallback 없음) 커스텀 CUDA op은 torch≤1.13+CUDA≤11.7을 요구하는데 이 머신 유일 toolkit은 CUDA 12.8 → `torch._check_cuda_version`가 **MAJOR 불일치(12≠11)로 hard-error**(§4.6 BEVFormer와 동일 근본원인)합니다. 문서 §3이 준 2안(Docker cu116 / blackwell 패치)이 **Docker 없고 정본 venv 오염 불가한 이 머신엔 둘 다 막혀** → **user-space cu117 툴체인을 sudo·Docker 없이 손조립**(nvcc V11.7.99[micromamba] + 실제 `libcudart.so.11.0`[pip] + Python.h[CPATH] + `CUDA_HOME` 조립)해 **`bev_pool_v2_ext.so` 9.13 MB 빌드 성공**. 2차 벽으로 카메라 전용 BEVDet인데 bundled mmdet3d가 **LiDAR-fusion 검출기 DAL을 eager import**해 spconv를 요구 → `spconv-cu117`로 해소. 🟡 **정직한 폴백**: 정식 detection 가중치가 **Baidu Pan 전용**(헤드리스 접근 불가)이라 `init_r50.pth`(torchvision backbone[실사전학습] + 랜덤 head)로 관통 → **mAP 0.0000 / NDS 0.0260은 버그가 아니라 예상값**(backbone만 진짜). **가중치와 무관한 latency는 유효**: FP32 forward **p50 34.06 ms**(batch1·event-timed)가 공식 README **33.3 ms(RTX 3090)**와 근접 교차확증. **검증된 것 = walking skeleton**(문서 §9 완주 기준: 툴체인·데이터[create_data 태그가 초안 `bevdetv2`가 아닌 **`bevdetv3`**, 323 train/81 val]·`bev_pool_v2` CUDA op[GPU 실행]·eval 하네스가 exit 0으로 관통). ⚠️ 절대 mAP/NDS는 init 가중치+mini 81장이라 문헌비교 불가. 실측 리포트 [`logs/stage8_capstone_report.html`](logs/stage8_capstone_report.html) · 벽·레시피·설계규칙 [`experiments/stage8_capstone/`](experiments/stage8_capstone/).
>
> 🔬 **2026-08-18(후속 세션), 그 위에 INT8/TRT-plugin까지 관통**시켰습니다 — 1차가 "다음 과제"로 남긴 **경로 A1**(`convert_bevdet_to_TRT.py --int8`, §4.6 BEVFormer와 동일한 "포크 커스텀 op 플러그인 툴체인" 벽)을 **user-space에 실제 조립해 6벽(W1~W6)을 순차 통과**했습니다: 커스텀 `TRTBEVPoolV2` 플러그인을 **풀 mmdeploy CMake 없이 2개 TU만 직접 컴파일**(W3, `libmmdeploy_tensorrt_ops.so` 58,896 B), torch 1.13 트레이서가 nested `QuickCumsumCuda`(symbolic 없는 CUDA autograd.Function)에서 `RuntimeError: _Map_base::at`+segfault로 죽는 걸 **export 전용 `new_zeros` shim**으로 우회(W5 — symbolic만 `mmdeploy::bev_pool_v2` 노드 emit·실연산은 추론 시 플러그인이 수행), deprecated `build_engine` segfault를 `build_serialized_network`로 교체(W6, 플러그인 탓 아닌 API 탓). → **FP32→FP16→INT8 TRT 엔진 3종**을 빌드해 지연 사다리 실측: **지연** 14.68→4.91(×2.99)→**2.63ms**(**×5.58**), **엔진** 245→90→**47MB**(×5.24↓), FP32 대비 출력 corr 0.985~1.000(height head 최저). 교차확증으로 TRT FP32 엔진 14.68ms가 PyTorch eager 34.06ms보다 **2.3× 빠름**. ⚠️ **캐비앗 불변**: init 가중치라 **절대 mAP≈0은 무의미** — 유효 산출물은 가중치-무관 **지연 사다리·엔진 크기·FP32대비 출력편차**뿐(정식 가중치 확보 시 동일 하네스로 정확도 재실행). INT8 관통 전용 리포트 [`logs/stage8_capstone_int8_report.html`](logs/stage8_capstone_int8_report.html) · 벽·레시피 [`experiments/stage8_capstone/capstone_constraints.md`](experiments/stage8_capstone/capstone_constraints.md) 벽 5.
>
> 🔬 **2026-08-20, 3·4단계를 Jetson AGX Orin 개발보드에서 온디바이스로 실측**했습니다(RTX 3080 프록시로는 불가능했던 것 — 3·5단계의 "trtexec 부재→polygraphy"와 3단계의 "DLA 범위 밖"을 실 보드가 해소). JetPack 6.2.1·**TensorRT 10.3.0.30**·**2× NVDLA v2**에서 같은 ResNet50을 JetPack 동봉 **실 `trtexec`**로 5엔진 관통했습니다: iGPU FP32→FP16→INT8(×1.91, 단 작은 Ampere iGPU라 **INT8≈FP16 0.984** — RTX 3080은 0.927), 그리고 **DLA INT8이 성능/와트 챔피언 51.29 inf/s/W**(iGPU INT8의 **1.547×**·전력 **절반**, 지연만 1.262× 느림). DLA는 **INT8 전용기**(레이어 배치가 INT8·FP16 동일한데 **DLA FP16이 13.87× 느림** — 순수 NVDLA 데이터패스)이고, 오프로드는 `tegrastats` GR3D가 iGPU 95%→DLA **3~16%로 붕괴**해 수치로 증명됩니다(DLA-후보 **2/2 오프로드**=conv 백본 120층+fc, GPU 폴백은 AVG-pool·flatten 2층뿐 — 이론의 "20~30% 폴백" 경계 훨씬 아래). 겸사 **§2-2 sign-flip 표에 Jetson A78AE(4번째·2번째 dotprod-ARM) 행**(38.47→18.22ms **×2.11**)을 채워 Pi 5 A76 패턴을 확증하고, 크로스플랫폼 발견을 **경로 의존**으로 정밀화했습니다 — **같은 MLAS SDOT 커널이면 INT8 예측이 비트 동일**(Jetson↔Pi5 1000/1000), 경로가 다르면 ~96%(결정적 대조: 같은 ORT의 Jetson↔x86은 958, 다른 ORT의 Jetson↔Pi5는 1000 → 갈림은 ISA/버전이 아니라 정수 dot-product 커널). [`05` §2.3](study_guide/05_tensorrt.md)·[`06` §2-3](study_guide/06_multi_soc.md) 반영. ⚠️ **DLA INT8은 `--int8` 암묵 캘리브라 지연·전력만 유효, 정확도 미주장**(정확도는 명시적 QDQ인 iGPU INT8·3단계 RTX); 지연 event-timed·batch1·MAXN 상대만. 실측 리포트 [`logs/stage3_jetson_orin_ondevice_report.html`](logs/stage3_jetson_orin_ondevice_report.html) · 데이터 [`experiments/stage3_tensorrt/jetson_ondevice/`](experiments/stage3_tensorrt/jetson_ondevice/).
