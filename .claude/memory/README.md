...
| [`stage3-jetson-ondevice-hands-on.md`](stage3-jetson-ondevice-hands-on.md) | **3·4단계 Jetson AGX Orin 온디바이스**(데이터 커밋 00fd97d·문서 1a8f073 푸시완료, ResNet50) — 실 `trtexec` 실존으로 3·5단계 "trtexec 부재→polygraphy" 반전·3단계 "DLA 범위 밖" 해소. **DLA INT8 = 성능/와트 챔피언 51.29 inf/s/W**(iGPU INT8의 **×1.547**·전력 절반, 지연만 1.262×)·**DLA는 INT8 전용기**(레이어 배치 동일한데 FP16 **13.87× 느림** = 순수 NVDLA v2 데이터패스)·오프로드 GR3D 95%→3~16%로 증명(DLA-후보 2/2, GPU폴백 2층뿐)·작은 Ampere iGPU **INT8≈FP16 0.984**(RTX3080 0.927). 05 §2.3 + 06 §2-3 반영, tech-reviewer 팬인 PASS. ⚠️ DLA INT8은 `--int8` 암묵 캘리브라 지연·전력만 유효, 정확도 미주장. **후속(커밋 c03c174 푸시완료): iGPU∥DLA 동시부하 = GPU-폴백 직렬화로 공짜 병렬 아님**(iGPU+DLA0 60.8%·DLA 27% 붕괴, DLA0+DLA1 **87.0%가 최적 66.07 inf/s/W**)·**nvpmodel MAXN→50W 리더 교차**(iGPU −29.4% vs DLA −2.9% → 50W서 DLA **+8.8%**), 05 §2.3 보강 + 06 신설 §2-4, tech-reviewer PASS(🔴0). **후속2 새 모델 축 DETR(2026-08-21, 커밋 9ef2a58 푸시완료)**: 무거운 트랜스포머는 iGPU INT8 이득 부활(INT8/FP16 0.710 vs ResNet50 0.984 부호반전)·explicit QDQ INT8 파싱불가(3단계 case C 재현)·DLA 트랜스포머 부적합(폴백 404/16조각=NVDLA v2 하드한계 → iGPU FP16의 30× 느림, DLA=CNN 가속기), 05 §2.3 DETR 콜아웃 순수삽입(8/0)+리포트 `logs/stage3_jetson_orin_detr_report.html`+`.../detr/`, tech-reviewer PASS(🔴0·🟡1수정). **후속3 정확도 축(2026-08-21, 커밋 d563439 푸시완료)**: 솔로 스윕 "정확도 미주장" 캐비앗을 닫음 — 저장 `.plan`을 4단계 CPU-프록시 같은 ResNet50 1000장 번들에 돌려 예측 1:1(결정타: `rn50_gpu_int8.plan`이 CPU 프록시와 같은 `resnet50_int8_qdq.onnx`에서 빌드→QDQ scale 동일→유일 변수=정수커널). accuracy-valid iGPU INT8(explicit QDQ) top-1 **0.7620=FP32 무손실>CPU MLAS 0.7500**(무음 FP32 폴백 아님)·**INT8 경로의존이 CPU↔가속기 경계 넘음**(TRT INT8 vs MLAS INT8 961/1000=4단계 Jetson↔x86 958 대역, ≠Pi5 100% 같은 커널)·FP32는 iGPU vs CPU 1000/1000·**implicit DLA INT8 0.017 붕괴**(같은 하네스 DLA FP16 0.7610→하네스버그 아님). 05 §2.3+06 §2-3 순수삽입+리포트 `logs/stage3_jetson_orin_accuracy_report.html`+`.../accuracy/`, tech-reviewer PASS(🔴0·🟡1 line-ref 876→892·net 05=9/1). ⚠️1000장 서브셋 상대만 |
