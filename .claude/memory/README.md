...
| [`stage4-arm-cpu-fallback-proxy.md`](stage4-arm-cpu-fallback-proxy.md) | **4단계 CPU 폴백 프록시**(부분·프록시, Raspberry Pi 5 Cortex-A76) — 벤더 NPU 대신 세 SoC 공통의 **ARM Cortex-A 폴백 경로**를 실측. 같은 ResNet50 INT8 QDQ·같은 ORT `CPUExecutionProvider`인데 **CPU ISA가 양자화 이득의 부호를 결정**: Pi dotprod(`asimddp`/SDOT) INT8 **×1.83 빠름** vs x86 no-VNNI **1.76× 느림**. 크로스플랫폼 예측 동일성은 FP32만(1000/1000)·INT8은 958/1000(42장 상이). golden 회귀는 결정론적 TRT 전용→CPU 4행 `cpu_proxy/`로 격리(하네스 6행·3 passed 유지). Pi는 NPU 아닌 폴백 바닥값(offload 0%), NPU 본실측은 보드 대기 |
