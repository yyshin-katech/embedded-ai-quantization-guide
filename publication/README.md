...
- **최대 약점:** 통계적 엄밀성(batch1·단일실행·서브셋·지연 CI 부재). *관련연구 미작성*은 **2026-09-04 해소** — §2 Related Work 작성 + BibTeX 46건 전량 웹 실검증 완료. *C2 결정성 축의 단일실행 우려*는 **2026-09-10 부분 해소** — PoT 실험서 2대×3정밀도×5회=**30 런 전부 비트동일**(`logits_md5` 편차 0)로 크로스디바이스 불일치가 스레드 노이즈 아닌 결정론적 ISA 차이임을 입증(게이트 수치는 점추정 아닌 정확값). 단 지연 CI는 여전히 미보강.

상세는 `publishability_assessment.html` 참조.

## 다음 단계

1. 논지 1개 확정(옵션 A) 후 핵심 비교 구성에 반복 실행 + 신뢰구간 재실행(아티팩트 재사용). ~~PoT 완화책 검증~~ **완료(2026-09-10)** — x86↔A76 INT8 주 게이트 NO-GO(958→869/919), §5/§10/§11·Appendix A·리포트 반영, 30-런 결정성.
2. ~~관련연구(MLPerf/MLPerf Tiny 등) + Threats to Validity 절 초안.~~ **완료(2026-09-04)** — §2·§11 작성, refs 46건 검증.
